...
2. **File order is not commit order.** Events are published by parallel shards, so the
   ...
   committed the events. The authoritative commit order is defined in §3 (R1).
...
| `type`     | string | One of: `account_open`, `deposit`, `withdrawal`, `transfer`, `adjustment`. |
...
the snapshot metadata (`duplicates_ignored`, §5).
...
- **`withdrawal`** — if `balance(account_id) < amount_cents`, the event MUST be **rejected**
  with reason `insufficient_funds` (recorded per §5, no state change). Otherwise
  `balance(account_id) -= amount_cents`.
- **`transfer`** — moves `amount_cents` from `src` to `dst` and charges the sender the fee
  ...
  `balance(src) < amount_cents + fee`, the event MUST be **rejected** with reason
...
- **`adjustment`** — `balance(account_id) += delta_cents`, applied unconditionally.
  Adjustments are back-office corrections and MAY drive a balance negative.

A balance may never go negative as the result of a `withdrawal` or `transfer`; the only
events permitted to produce a negative balance are `adjustment` events.
...
### R6 — Rejection recording

Every rejected event MUST be recorded exactly once in the `rejected_events` table
(§5) with its `event_id`, its `type`, and the reason string. Retransmissions of a rejected
event are deduplicated like any other event (R2) and are not recorded again.
...
`accounts` MUST contain one row per `account_open` in the log, with the final balance after
the full replay.
